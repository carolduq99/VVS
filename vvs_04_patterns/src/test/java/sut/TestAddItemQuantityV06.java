 ...
 * Refactoring a Test.
 * 
 * Now that we have cleaned up the result verification logic, let’s turn our
 * attention to the finally block at the end of the test.
 * 
 * the finally block calls the deleteObject method on each of the objects
 * created by the test. Unfortunately, this code suffers from a fatal flaw. Have
 * you noticed it yet?
 * 
 * What happens if the first call to deleteObject throws an exception? As coded
 * here, none of the other calls to deleteObject would be executed. The solution
 * is to use a nested try/finally block around this first call, thereby ensuring
 * that the second call to deleteObject always executes. But what if the second
 * call fails?
 * 
 ...
 * @version $Id: TestAddItemQuantityV06.java 290 2016-03-13 14:23:37Z vv $
 */
public class TestAddItemQuantityV06 {
