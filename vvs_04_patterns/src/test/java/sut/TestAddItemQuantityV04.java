 ...
 * @version $Id: TestAddItemQuantityV04.java 290 2016-03-13 14:23:37Z vv $
 */
public class TestAddItemQuantityV04 {
	...
	public void testAddItemQuantity_severalQuantity() {
		...
		try {
			...
			if (lineItems.size() == 1) {
				...
				assertEquals("invoice", expected.getInv(), actualItem.getInv());
				assertEquals("product", expected.getProd(), actualItem.getProd());
				assertEquals("quantity", expected.getQuantity(), actualItem.getQuantity());
				assertEquals("discount", expected.getPercentDiscount(), actualItem.getPercentDiscount());
				assertEquals("unit pr", new BigDecimal("19.99"), actualItem.getUnitPrice());
				assertEquals("extend pr", new BigDecimal("69.965"), actualItem.getExtendedPrice());
			} else {
